...
const initial = {
  fade:{opacity:'0'}, 'fade-up':{opacity:'0',transform:'translateY(16px)'},
  ...
  'zoom-in':{opacity:'0',transform:'scale(.96)'}, 'zoom-out':{opacity:'0',transform:'scale(1.04)'}
};
const fin = {
  fade:{opacity:'1',transform:'none'}, 'fade-up':{opacity:'1',transform:'none'},
  'fade-down':{opacity:'1',transform:'none'}, 'fade-left':{opacity:'1',transform:'none'},
  'fade-right':{opacity:'1',transform:'none'}, 'zoom-in':{opacity:'1',transform:'none'},
...
function prep(el, onceDefault){
  if(el.__cgObserved__) return; el.__cgObserved__=true;
  ...
  el.__cgOnce__ = once; el.__cgRepeat__ = !once;
  setStyle(el, { willChange:'opacity, transform',
    transition:`opacity ${duration}ms ${easing}, transform ${duration}ms ${easing}`,
    ...(initial[type] ?? {opacity:'0'}) });
  if (delay) el.style.transitionDelay = `${delay}ms`;
  const stagger = num(el.dataset.cgStagger,0);
  if (stagger) el.querySelectorAll(':scope > *').forEach((c,i)=>{
  ...
  init(opts = {}){
    ...
    const io = new IntersectionObserver((entries)=>{
      for(const e of entries){
        ...
        if(e.isIntersecting){
          setStyle(el, fin[t] ?? {opacity:'1',transform:'none'});
          el.classList.add('cg-in'); el.dispatchEvent(new CustomEvent('cg:enter'));
          if(el.__cgOnce__) io.unobserve(el);
        }else if(el.__cgRepeat__){
          setStyle(el, initial[t] ?? {opacity:'0'}); el.classList.remove('cg-in');
